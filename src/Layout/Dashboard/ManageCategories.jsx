...
const ManageCategories = () => {
    ...
    try {
      ...
      if (editMode) {
        ...
        if (res.data.modifiedCount > 0) {
          setCategories(
            categories.map((cat) =>
              cat._id === editingCategoryId ? { ...cat, ...categoryData } : cat
            )
          );
          Swal.fire({
            position: "top-end",
        ...
        if (res.data.insertedId) {
          setCategories([
            ...categories,
            { ...categoryData, _id: res.data.insertedId },
          ]);
          Swal.fire({
            position: "top-end",
            icon: "success",
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Helmet>
        <title>CareNext Pharamacy | Dashboard | Manage Category</title>
      </Helmet>
            ...
            <tbody>
              {categories.map((category, index) => (
                <tr key={category._id} className="border-b">
                  ...
                  <td className="py-2 px-4 text-center">
                    <img
                      className="w-10 h-10 object-contain mx-auto rounded-xl"
