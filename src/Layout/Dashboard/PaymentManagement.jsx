...
const PaymentManagement = () => {
    ...
    const fetchPayments = async () => {
      try {
        const response = await axios.get(
          "https://y-plum-nine.vercel.app/payments"
        );
  ...
  const handleAcceptPayment = async (id) => {
    try {
      await axios.patch(`https://y-plum-nine.vercel.app/payments/${id}`, {
        status: "paid",
      });
  ...
  return (
    <div className="container mx-auto p-4 border-2 rounded-xl">
      <Helmet>
        <title>CareNext Pharamacy | Dashboard | Payment Management</title>
      </Helmet>
      <h2 className="text-2xl font-bold mb-4 border-y-2 p-2  text-center">
        Payment Management
      </h2>
